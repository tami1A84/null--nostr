...
class NostrRepository(
     ...
     */
    suspend fun fetchRelayTimeline(relayUrl: String, limit: Int = 50): List<ScoredPost> =
        withContext(Dispatchers.IO) {
            try {
                val myPubkey = prefs.publicKeyHex ?: ""
                ...
                val rootPosts = events.filter { it.getTagValues("e").isEmpty() }
                // プロファイルのみ取得する軽量エンリッチ（reactions/zaps不要）
     ...
     * Rust エンジンは全リレーにブロードキャストするため、単一リレー取得にはこちらを使う。
     */
    private suspend fun fetchFromSingleRelayWs(
        relayUrl: String,
        limit: Int,
     ...
     * キャンセル時に WebSocket を閉じる。
     */
    fun openRelayLiveStream(relayUrl: String): Flow<NostrEvent> = callbackFlow {
     ...
     */
    private suspend fun enrichPostsLight(events: List<NostrEvent>): List<ScoredPost> {
