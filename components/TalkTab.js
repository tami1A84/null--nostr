...
const TalkTab = forwardRef(function TalkTab({ pubkey, pendingDM, onDMOpened }, ref) {
  ...
  const isNosskey = loginMethod === 'nosskey'
  ...
  // Chat list view
  if (!selectedChat) {
    // Show notice if DM is not supported (but only if not navigating from profile)
      ...
      return (
        <div className="min-h-screen pb-16">
          ...
          <div className="px-4 py-16 text-center">
            ...
            <h3 className="font-semibold text-[var(--text-primary)] mb-2">DM機能を有効にする</h3>
            {isNosskey ? (
              <>
                <p className="text-sm text-[var(--text-secondary)] mb-4">
                  パスキーログイン中はミニアプリ画面で<br/>
                  秘密鍵をエクスポートするとDMが使えます。
                </p>
                <p className="text-xs text-[var(--text-tertiary)]">
                  ミニアプリ → パスキー設定 → 秘密鍵をエクスポート
                </p>
              </>
            ) : (
              <p className="text-sm text-[var(--text-secondary)] mb-4">
