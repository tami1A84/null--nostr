...
const TimelineTab = forwardRef(function TimelineTab({ pubkey, onStartDM }, ref) {
  ...
  // Reload timeline when mode changes
  useEffect(() => {
    if (!loading) {
      loadTimeline()
    }
  ...
  const loadFollowList = async () => {
    ...
    try {
      const follows = await fetchFollowList(pubkey, getReadRelays())
  ...
  const loadTimeline = async () => {
    ...
    try {
      // Build filter based on timeline mode
      let noteFilter = { kinds: [1], since: oneHourAgo, limit: 100 }
      let repostFilter = { kinds: [6], since: oneHourAgo, limit: 50 }
      
      // For following mode, filter by followed authors
      if (timelineMode === 'following' && followList.length > 0) {
        noteFilter.authors = followList
        repostFilter.authors = followList
      } else if (timelineMode === 'following' && followList.length === 0) {
        // No follows - show empty state
        setPosts([])
        setLoading(false)
        return
      }
      
      // Fetch notes and reposts in parallel for faster loading (1 hour only)
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`px-3 py-1.5 text-sm rounded-full transition-all ${
                timelineMode === 'global'
                  ? 'bg-[var(--line-green)] text-white font-medium'
