...
// Default muted pubkeys (converted from npub)
export const DEFAULT_MUTED_PUBKEYS = [
  '7f93c3b88fa5d7ab0ed5a5e39649255cbefc5295bd506bf25d08844135ab5401', // npub107f7sac37ht6k8dt6jncjxcj2uhltu2044pz7ljt55yepx45k4qsg643c4
  'ae2df419ce51e5cba5531da3952d033dc8085557673434c04a9d6b93bcc63e8f', // npub14cklgree5w09ju4f36dw55rgn78gzy4an6pnjcsz48ttjw7v050sv5jptq
  'c56319e6353b736c5d9c3bb6e979648cb2b4b582c5067abb3c59d6539658b89f'  // npub1c4vv0nrfh0drcfchs2mndw4uvjkd9k393v20x4kme2txev9hhz0qw4cqk7
]
...
// Sign event via NIP-07, Nosskey, or private key
...
export async function decryptNip44(pubkey, ciphertext) {
  ...
  // Fall back to NIP-07
  if (!hasNip07()) {
    throw new Error('復号機能が利用できません。ミニアプリ画面で秘密鍵をエクスポートしてください。')
  }
  try {
...
export async function fetchEvents(filter, relays = [DEFAULT_RELAY]) {
  ...
  if (useRxNostr && typeof window !== 'undefined') {
    return rxFetchEvents(filter, relays)
  }
...
export function subscribeToEvents(filter, relays, onEvent, onEose) {
  // Use rx-nostr if enabled
  if (useRxNostr && typeof window !== 'undefined') {
    return rxSubscribeToEvents(filter, relays, onEvent, onEose)
  }
...
export async function publishEvent(event, relays = WRITE_RELAYS) {
  // Use rx-nostr if enabled
  if (useRxNostr && typeof window !== 'undefined') {
    return rxPublishEvent(event, relays)
  }
...
export async function fetchMuteList(pubkey) {
  try {
    ...
    // Start with default muted pubkeys
    let pubkeys = [...DEFAULT_MUTED_PUBKEYS]
    let eventIds = []
    let hashtags = []
    let words = []
    let rawEvent = null
    ...
    if (events.length > 0) {
      ...
      // Add user's muted pubkeys (avoiding duplicates)
      const userPubkeys = event.tags.filter(t => t[0] === 'p').map(t => t[1])
      for (const pk of userPubkeys) {
    ...
    // Return default muted pubkeys even on error
    return { pubkeys: [...DEFAULT_MUTED_PUBKEYS], eventIds: [], hashtags: [], words: [] }
  }
}
