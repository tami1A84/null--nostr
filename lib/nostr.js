...
import {
  ...
  getCachedFollowList,
  setCachedFollowList,
  getCachedMutualFollows,
  setCachedMutualFollows
} from './cache'
...
export function setStoredPrivateKey(pubkey, privateKeyHex) {
  return KeyStore.storePrivateKey(pubkey, privateKeyHex)
}
...
export function clearStoredPrivateKey(pubkey) {
  KeyStore.clearPrivateKey(pubkey)
}
...
export async function signEventNip07(event) {
  ...
  if (hasStoredPrivateKey() && getAutoSignEnabled()) {
    try {
      const secretKey = KeyStore.getPrivateKeyBytes()
      if (secretKey) {
  ...
  if ((loginMethod === 'bunker' || loginMethod === 'nostr-connect') && window.bunkerSigner) {
    try {
  ...
  // Fallback to NIP-07 (this also works for bunker via window.nostr and nostr-login)
  if (!hasNip07()) {
    ...
    try {
      ...
      if (keyInfo) {
        ...
        if (privateKeyHex && storedPubkey) {
          KeyStore.storePrivateKey(storedPubkey, privateKeyHex)
        }
      }
    } catch (e) {
 ...
 * @param {Object} event - Kind 9735 event
 * @returns {Object|null} Parsed zap info
 */
export function parseZap(event) {
  ...
  try {
    ...
    return {
      id: event.id,
      pubkey: zapRequest.pubkey, // Sender
      recipient: event.pubkey, // Receiver
      amount: Math.floor(amountMsats / 1000), // Amount in sats
      comment: zapRequest.content || '',
      targetEventId: zapRequest.tags.find(t => t[0] === 'e')?.[1],
...
/**
 * Fetch mutual follows with caching
 */
export async function fetchMutualFollowsCached(pubkey, relays = [getDefaultRelay()], forceRefresh = false) {
  if (!forceRefresh) {
    const cached = getCachedMutualFollows(pubkey)
    if (cached) return cached
  }

  const mutuals = await fetchMutualFollows(pubkey, relays)
  setCachedMutualFollows(pubkey, mutuals)
  return mutuals
}
...
/**
 * Fetch people who follow a user (Kind 3 with #p tag)
 * @param {string} pubkey - User's pubkey
 * @param {string[]} relays - Relays to query
 * @returns {Promise<string[]>} Array of follower pubkeys
 */
export async function fetchFollowers(pubkey, relays = [getDefaultRelay()]) {
  try {
...
/**
 * Fetch mutual follows for a user
 * @param {string} pubkey - User's pubkey
 * @param {string[]} relays - Relays to query
 * @returns {Promise<string[]>} Array of mutual follow pubkeys
 */
export async function fetchMutualFollows(pubkey, relays = [getDefaultRelay()]) {
  try {
    const [follows, followers] = await Promise.all([
      fetchFollowList(pubkey, relays),
      fetchFollowers(pubkey, relays)
    ])

    const followSet = new Set(follows)
    return followers.filter(pk => followSet.has(pk))
  } catch (e) {
    console.error('Failed to fetch mutual follows:', e)
    return []
  }
}
    ...
    try {
      ...
      if (keyInfo) {
        ...
        if (privateKeyHex && currentPubkey) {
          KeyStore.storePrivateKey(currentPubkey, privateKeyHex)
        }
      }
    } catch (e) {
...
export async function searchNotes(query, options = {}) {
  ...
  try {
    const filter = {
      kinds: [1, NOSTR_KINDS.SHORT_VIDEO],
      search: query,
      limit
    }
...
// Directory relays for fetching NIP-65 relay lists (Outbox Model)
export const RELAY_LIST_DIRECTORY_RELAYS = [
 ...
 * Uses directory relays (wss://directory.yabu.me, wss://purplepag.es) by default
 * for better relay list discovery as per Outbox Model
 * @param {string} pubkey - User's public key
 * @param {string[]} relays - Relays to query (defaults to directory relays)
 * @returns {Promise<{read: string[], write: string[], all: Array<{url: string, read: boolean, write: boolean}>}>}
 */
export async function fetchRelayListMetadata(pubkey, relays = RELAY_LIST_DIRECTORY_RELAYS) {
  try {
    const events = await fetchEvents(
...
export async function publishRelayListMetadata(relayList) {
  ...
  try {
    ...
    // Publish to directory relays for Outbox Model discovery
    // plus user's own write relays
    const publishRelays = [
      ...RELAY_LIST_DIRECTORY_RELAYS,
      getDefaultRelay(),
      ...relayList.filter(r => r.write !== false).map(r => r.url)
    ]
    const uniqueRelays = [...new Set(publishRelays)].filter(isValidRelayUrl)

    const success = await publishManaged(signedEvent, uniqueRelays)
