...
export function generateRelayListByLocation(userLat, userLon) {
  // Find nearest relays
  const nearestRelays = findNearestRelays(userLat, userLon, 15)
  ...
  // Select inbox relays (3-4): prioritize LOCAL relays for receiving mentions
  // This is important so people in your region can reach you easily
  const inbox = []

  // Add nearest priority 1 relays first (local relays)
  nearestRelays
    .filter(r => r.priority === 1)
    .slice(0, 3)
  ...
  const globalRelay = GPS_RELAY_DATABASE
    .filter(r => r.priority === 1 && r.region === 'Global' && !inbox.some(i => i.url === r.url))
    .slice(0, 1)
  inbox.push(...globalRelay.map(r => ({ ...r, distance: calculateDistance(userLat, userLon, r.lat, r.lon) })))
  ...
  nearestRelays.slice(0, 2).forEach(r => {
    if (!addedUrls.has(r.url)) {
...
export async function autoDetectRelays() {
  try {
    ...
    // Find nearest region for display
    let nearestRegion = null
    let minRegionDist = Infinity
    for (const region of REGION_COORDINATES) {
      if (region.id === 'global') continue
      const dist = calculateDistance(location.lat, location.lon, region.lat, region.lon)
