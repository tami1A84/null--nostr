...
import { getPool, resetPool } from './connection-manager'

const NIP46_KIND = 24133
const DEFAULT_NIP46_RELAYS = ['wss://relay.damus.io', 'wss://nos.lol']

// NIP-46 request timeout (60 seconds for user approval)
const NIP46_CONNECT_TIMEOUT = 60000
// Normal request timeout (30 seconds)
const NIP46_REQUEST_TIMEOUT = 30000
...
async function sendNip46Request(method, params, session) {
  ...
  // Publish request with error handling
  try {
    const publishPromises = session.relays.map(relay =>
      pool.publish([relay], event).catch(e => {
        console.warn(`[NIP-46] Publish to ${relay} failed:`, e.message)
        throw e
      })
    )
    await Promise.any(publishPromises)
  } catch (e) {
    console.error('[NIP-46] Failed to publish to any relay:', e)
    throw new Error('NIP-46リクエストの送信に失敗しました')
  }
...
export async function connectWithBunkerUrl(bunkerUrl) {
  ...
  // Publish connect request with error handling
  try {
    ...
    console.error('[NIP-46] Failed to publish connect request:', e)
    throw new Error('接続リクエストの送信に失敗しました')
  }

  // Wait for ACK with proper cleanup
  return new Promise((resolve, reject) => {
    let sub = null
    let isResolved = false
    ...
    const timeout = setTimeout(() => {
      if (!isResolved) {
        cleanup()
        reject(new Error('接続がタイムアウトしました。Nostr Connect アプリで承認してください。'))
      }
      ...
      }],
      {
        onevent: async (ev) => {
          ...
          try {
            ...
            console.error('[NIP-46] Failed to process connect response:', e)
          }
        }
      }
    )
