...
const BANDWIDTH_CHECK_INTERVAL = 60000 // 1分ごとに再チェック

// 低帯域モードのしきい値 (1.5Mbps = 1500kbps)
const LOW_BANDWIDTH_THRESHOLD = 1500
...
export function estimateConnectionSpeed() {
  ...
  if (connection) {
    // downlink は Mbps 単位
    const speedKbps = (connection.downlink || 10) * 1000
    estimatedBandwidth = speedKbps
...
export function isLowBandwidth() {
  // キャッシュされた値が古い場合は再チェック
  if (Date.now() - lastBandwidthCheck > BANDWIDTH_CHECK_INTERVAL) {
    estimateConnectionSpeed()
  }

  if (estimatedBandwidth !== null) {
    return estimatedBandwidth < LOW_BANDWIDTH_THRESHOLD
  }

  // Connection API が使えない場合、saveDataモードをチェック
 ...
 * 画像サイズを帯域に応じて最適化
 */
export function getOptimalImageSize(originalWidth, originalHeight) {
  const lowBandwidth = isLowBandwidth()

  // 低帯域モードでは最大サイズを制限
  const maxWidth = lowBandwidth ? 480 : 1024
  const maxHeight = lowBandwidth ? 480 : 1024
...
export function getOptimalQuality() {
  return isLowBandwidth() ? 0.6 : 0.85
}
...
// ============================================
// 遅延読み込み・低帯域最適化ユーティリティ
// ============================================

/**
 * Intersection Observer を使った画像遅延読み込みを設定
...
export function setupLazyImage(img, src, options = {}) {
  ...
  // IntersectionObserver が使えない場合は即座に読み込み
  if (typeof IntersectionObserver === 'undefined') {
    loadImage(img, src, onLoad, onError)
    return
  }
 ...
 * 画像を読み込む内部関数
 */
function loadImage(img, src, onLoad, onError) {
  // 低帯域モードの場合、プロキシ経由で小さい画像を取得
  const optimizedSrc = isLowBandwidth() ? getProxiedUrl(src) : src
...
export function preloadImage(url, options = {}) {
  const { skipOnLowBandwidth = true } = options

  if (skipOnLowBandwidth && isLowBandwidth()) {
...
/**
 * 複数画像のプリロード（優先度付き）
 * @param {string[]} urls - 画像URLの配列
 * @param {Object} options - オプション
 */
export async function preloadImages(urls, options = {}) {
  const {
    maxConcurrent = isLowBandwidth() ? 1 : 3,
    onProgress = null
  } = options
  ...
  for (let i = 0; i < urls.length; i += maxConcurrent) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map(url => preloadImage(url, { skipOnLowBandwidth: false }))
    )
...
/**
 * 画像URLを低帯域用に最適化
 * wsrv.nl プロキシを使用してリサイズ
 */
export function getOptimizedImageUrl(url, options = {}) {
  const {
    width = isLowBandwidth() ? 320 : 640,
 ...
 * アバター画像の最適化されたURLを取得
 */
export function getOptimizedAvatarUrl(url, size = 48) {
  if (!url) return null

  const optimizedSize = isLowBandwidth() ? Math.min(size, 64) : size

  return getOptimizedImageUrl(url, {
    width: optimizedSize * 2, // Retina対応
    quality: 0.8,
    format: 'webp'
  })
}
...
/**
 * Blurhash プレースホルダー生成（将来の拡張用）
 * 現在は単純なグラデーションを返す
 */
export function generatePlaceholder(aspectRatio = 1) {
  const width = 32
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}">
      <rect fill="var(--bg-tertiary)" width="${width}" height="${height}"/>
    </svg>
  `
...
/**
 * 動画ファイルをアップロード（リトライ付き、タイムアウト延長）
 * @param {File} file - 動画ファイル
 * @param {Object} options - アップロードオプション
 * @returns {Promise<string>} アップロードされた動画のURL
 */
export async function uploadVideoWithRetry(file, options = {}) {
  const {
    maxRetries = UPLOAD_CONFIG.retry.maxAttempts,
    timeout = 120000 // 動画は2分のタイムアウト
  } = options

  return uploadImageWithRetry(file, { maxRetries, timeout })
}
