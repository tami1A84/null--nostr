...
function calculateTimeDecay(createdAt) {
  ...
  if (ageInHours > TIME_DECAY.maxAge) {
    return 0.1
  }
...
export async function fetchEngagementData(eventIds, relays) {
  ...
  try {
    // Fetch reactions, reposts, and replies in parallel
    const [reactions, reposts, replies, zaps] = await Promise.all([
      ...
      fetchEventsManaged(
        { kinds: [NOSTR_KINDS.REPOST], '#e': eventIds, limit: 500 },
        relays,
        { timeout: 10000 }
      ),
      fetchEventsManaged(
        { kinds: [NOSTR_KINDS.TEXT_NOTE], '#e': eventIds, limit: 500 },
        relays,
        { timeout: 10000 }
      ),
      fetchEventsManaged(
    ...
    for (const id of eventIds) {
      engagement[id] = { likes: 0, reposts: 0, replies: 0, zaps: 0 }
    }
    ...
    for (const event of replies) {
      ...
      if (replyTo && engagement[replyTo]) {
        engagement[replyTo].replies++
      }
    }
...
export function getRecommendedPosts(allPosts, options = {}, limit = 50) {
  ...
  // Final sort by score to interleave categories naturally
