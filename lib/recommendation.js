...
import { NOSTR_KINDS, ENGAGEMENT_WEIGHTS, NEGATIVE_WEIGHTS, SOCIAL_BOOST, TIME_DECAY } from './constants'
...
export function getRecommendedPosts(allPosts, options = {}, limit = 50) {
  ...
  const candidatePosts = allPosts.filter(post => {
    ...
    const profile = options.profiles?.[post.pubkey]
    if (!profile) return false // No profile data yet, safer to exclude from recommendation
    if (!profile.picture) return false
    if (!profile.name && !profile.displayName) return false
