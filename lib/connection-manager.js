...
function isRelayInCooldown(relay) {
  ...
  const now = Date.now()
  const cooldownExpired = now - failureInfo.lastFailure > CONFIG.failedRelayCooldown
  ...
  return failureInfo.failures >= CONFIG.maxFailuresBeforeCooldown
}
...
export function filterAvailableRelays(relays) {
  ...
  // If all relays are in cooldown, return original list (better to try than give up)
...
function startHealthCheck() {
  ...
  healthCheckTimer = setInterval(() => {
    ...
    for (const [relay, info] of failedRelays.entries()) {
      if (now - info.lastFailure > CONFIG.failedRelayCooldown) {
        failedRelays.delete(relay)
      }
    }
...
export async function executeWithRetry(queryFn, relays, options = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    // Skip relay if it's in cooldown (might have been added during retry)
    ...
    try {
      ...
      // Only log in development mode or if not silent
